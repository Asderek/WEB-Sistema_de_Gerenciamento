<?php
	$nomeDoArquivo = "mainMonitor.php";
	$AZUL_ESCURO = "#428BCA";
	$AZUL_CLARO = "#A4DDFF";
	$VERDE_CLARO = "#90FF90";
	$VERMELHO_ESCURO = "#A02020";
	
	//echo "matriculaResponsavel = $matriculaResponsavel<br>";
	
	include ('../../newconexao.php');
	include ('calendarioMonitores.php');
	
	$nome = PROFESSORES_GETNAME($matriculaResponsavel);
	//echo "nome = $nome<br>";
						...
						for($dias=0;$dias<7;$dias++)
						{
							...
							{//sqlBloqueio
								...
								if ($contDias<10)
									...
									$sqlDia = $contDias;
								$sqlBloqueio = "SELECT * FROM `calendario` WHERE `dia` = \"$sqlDia\" AND `mes` = \"$displaySqlMonth\" AND `ano` = \"$ano\" AND `bloqueado` = 1";
