<?php
	...
	$sqlAbr = "SELECT * FROM horariosplantao WHERE matricula = $matricula";
	$queryAbr = $conexao->query($sqlAbr);
	$rowsAbr = $queryAbr->fetchAll( PDO::FETCH_ASSOC );
	...
	echo "<button class=\"btn\" onclick=\"printData()\">Imprimir Relatorio</button>";
	echo '<h3 class="text-center">'.$abr.'</h3>';
			...
			if(count($dias[$day][$hour])>0)
			{
				...
				for($aluno=0;$aluno<count($dias[$day][$hour]);$aluno++)
				{
					...
					for($atr=0;$atr<count($dias[$day][$hour][$aluno]);$atr++)
					{
						...
						if($atr == 1)
						{
							$matAluno = $dias[$day][$hour][$aluno][$atr-1];
							$nome = $dias[$day][$hour][$aluno][$atr];
							echo "<td align=\"center\"><button type=\"submit\" name=\"mode\" value=\"veraluno$matAluno\" class=\"button-link\"> $nome </button></td>";
						}
						else
