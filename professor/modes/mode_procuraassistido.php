<?php
	...
	//echo "nomeProfessor = $nomeProfessor<br>";
	...
	//echo "nomeAssistido = $nomeAssistido<br>";
	//echo "escolha = $escolha<br>";
	$nomes = array();
	$atendimento = false;
	if ($escolha == 1)
	{
		$sqlSearch = "SELECT * FROM `atendimentos` WHERE   `responsavel` = \"$nomeProfessor\" AND (`nome` LIKE \"%$nomeAssistido%\" OR `autor`  LIKE \"%$nomeAssistido%\" OR `beneficiado` LIKE \"%$nomeAssistido%\" ) ORDER BY `nome` ASC";
	}
	else
		$sqlSearch = "SELECT * FROM `atendimentos` WHERE `reu` LIKE \"%$nomeAssistido%\" AND  `responsavel` = \"$nomeProfessor\" ORDER BY `nome` ASC";
