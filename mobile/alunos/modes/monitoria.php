<form action="cadastramonitoria.php" method="post">
	<?php 
		...
		{
			echo "<h4 class=\"text-center\">Candidatura à monitoria<br> cadastrada com sucesso</h4>";
		}
	?>
	...
	<?php
		...
		for($x=0;$x<count($rowsSearch);$x++)
		{
			...
			if($status==1)
			{
				...
				$sqlDuplicated = "SELECT * FROM inscritosmonitoria WHERE matricula = $matricula AND professor = \"$name - $area\"";
				echo "sqlDuplicated = $sqlDuplicated<br>";
				
				$queryDuplicated = $conexao->query($sqlDuplicated);
				if ($queryDuplicated != false)
				{
					$resultDuplicated = $queryDuplicated->fetchAll(PDO::FETCH_ASSOC);
					if (count($resultDuplicated) > 0)
					{
						echo '<option name="'.$x.'"  disabled>'.$name.' - '.$area.'</option>';
					}
					else
